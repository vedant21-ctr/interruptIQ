...
export function DemoPanel({
  ...
  onTrigger: (ctx: any, notif: string, exp: string) => void;
}) {
  return (
    <div className="glass-card p-6 flex flex-col justify-center relative overflow-hidden group">
      ...
      <div className="flex items-center gap-2 mb-6">
        ...
        <span className="text-xs uppercase tracking-[0.2em] font-black text-violet-400">
          Demo Scenarios
        </span>
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 relative z-10">
        {SCENARIOS.map((s, i) => (
          ...
          >
            <span className="text-3xl mb-2 drop-shadow-md group-hover:scale-110 transition-transform duration-300">
              {s.icon}
            </span>
